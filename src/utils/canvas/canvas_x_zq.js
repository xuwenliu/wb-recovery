...
var muban = function (HB, VB, STH, STV) {
  ...
  this.STH = STH;
  this.STV = STV;
  this.txt_arr_num = ['4', '3', '2', '1', '0']; //左侧数字
  this.txt_arr_t = ['独立完成', '少许协助', '部分反应', '少许反应', '无反应']; //左侧文字
  ...
  this.box = function (ctx, num_v, num_h) {
    ...
    for (var i = 0; i < this.txt_arr_t.length; i++) {
      ...
      ctx.fillText(this.txt_arr_t[i], 0 * this.HB + this.STH - 60, i * this.VB + this.STV);
    }
    ctx.closePath();
  };
  ...
  this.line = function (ctx, num_v, num_h, name_score, fix_score, v_or_h, name2) {
    ...
    for (var i = 0; i < num_v; i++) {
      ...
      if (fix_score != null) {
        var y = parseInt(
          (1 - name_score[i].score / fix_score[name_score[i].name]) * 4 * this.VB + this.STV,
        );
      } else {
        var y = parseInt((1 - name_score[i].score / 4) * 4 * this.VB + this.STV);
      }
      ...
      if (v_or_h == 1) {
        //1竖写
				var LEN = 8;
				var name_sort = '';
        if (name_score[i].name.length >= LEN) {
          name_sort = name_score[i].name.substring(0, LEN) + '...';
        } else {
          name_sort = name_score[i].name;
        }
        //				console.log(name_score[i].name)
        //				name_sort = name_score[i].name.substring(0,5);
      ...
      if (fix_score != null && fix_score[name_score[i].name] != undefined) {
        ...
        ctx.fillText((fix_score[name_score[i].name] * 4) / 4, x - 5, 0 * this.VB + this.STV + 10); //上-数字
        ctx.fillText((fix_score[name_score[i].name] * 3) / 4, x - 5, 1 * this.VB + this.STV + 10); //中-数字
        ctx.fillText((fix_score[name_score[i].name] * 2) / 4, x - 5, 2 * this.VB + this.STV + 10); //中-数字
        ctx.fillText((fix_score[name_score[i].name] * 1) / 4, x - 5, 3 * this.VB + this.STV + 10); //下-数字
      }
    }
    //		console.log(name2)
    //		console.log(v_or_h)
