...
const ColumnLocation = (props) => {
  ...
  const queryParentSectionAll = async () => {
    ...
    if (common && res) {
      ...
      const newCommon = commonArr.map(item => {
        item.list = [];
        res.forEach(sub => {
        ...
        >
...

          {
            parentSection.map(item => {
              return <Col key={item.ordianl} span={6}><Form.Item {...formItemLayout} label={item.codeCn} name={item.codeEn}>
                <Select onChange={(e) => selectChange(e, item)} dropdownRender={menu => (
                  <div>
                    {menu}
                  ...
                  {
                    item.list.map(sub => <Select.Option key={sub.id} value={sub.id}>{sub.name}</Select.Option>)
                  }
...
export default connect(({ functionAndColumnLocation, loading }) => ({
  functionAndColumnLocation,
  loading: loading.effects['functionAndColumnLocation/fetchAdvanced'],
}))(ColumnLocation);
